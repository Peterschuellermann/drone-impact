...
2. **Per-point scoring parallelism (ThreadPoolExecutor):** `ScoringEngine._score_points_parallel` distributes evaluation points across threads. Each point gets an independent RNG via `SeedSequence.spawn()` for deterministic results regardless of thread scheduling. Physics kernels are compiled with Numba `@njit(parallel=True, cache=True)`, which releases the GIL during execution. This makes ThreadPoolExecutor effective for per-point parallelism. Default is `point_workers: 0` (auto = cpu_count).
